...
void Timeout()
{
    while (true) {
        for (map<int, shared_ptr<Session>>::iterator it = gSessions.begin(); it != gSessions.end();) {
            if (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - it->second->lastUseTime).count() > 10000) {
                cout << 'User' << it->first << " left: Timeout";
...
//    while (true)
    {
        ...
        case M_INIT:
        {
            ...
            pSession->lastUseTime = std::chrono::system_clock::now();
        ...
        case M_GETDATA:
        {
            gSessions.find(m.m_Header.m_From)->second->lastUseTime = std::chrono::system_clock::now();
